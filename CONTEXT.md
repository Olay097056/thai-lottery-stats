...
The top-level letter (A, B, C, D, E, F/Codex, G, H, I, J/เจ้าสัว, HM/Hermes — รวม HM2/HM3 ท้าย3 ซึ่งอยู่ในกลุ่ม HM ด้วย) — not the individual sub-formula (e.g. D1–D4, I1/I2). Two sub-formulas from the *same* letter group agreeing does not count as เลขแนะนำ agreement, since they share the same underlying method. Group key resolution: explicit `fr.group` field wins (Hermes carries `'HM'`), else the first character of the formula name (J1/J2 → 'J', X1.. → 'X') — see PRD-hermes-pool-1-4.md for why 'HM' must not collapse into 'H' (มิสเตอร์ซี).

**Field type**:
The exact prize-slot shape a formula's output is checked against in backtest — `bottom2`, `bottom2_unit`, `top3`, `front3`, `back3`, `back3exact`, `prize1_last2`, `prize1_last4_digits`, `pool6`, `pool6_14`, `pool14_tail3`. เลขแนะนำ agreement requires an exact field-type match, not just equal digit-length (e.g. a `front3` guess and a `back3exact` guess of the same digits do NOT count as agreeing, even though Hit tracking for ordinary 3-digit Picks already treats all five 3-digit slots as interchangeable).
_Avoid_: field, format (use "field type" for this specific backtest-comparability tag)

**Hermes ท้าย3 (HM2/HM3)**:
Two 3-digit-tail formulas in the Hermes family (group `HM`, PRD-hm2-hermes-tail3.md): HM2 · Hermes ท้าย3 ใต้ดิน targets the last-3-digits of any number in the target draw's prize 1–4 pool (field `pool14_tail3` — underground-style bet; the name and card note mark "ใต้ดิน" so users don't mistake it for an official prize); HM3 · Hermes ท้าย3 ทางการ targets the official เลขท้าย 3 ตัว prize numbers (`back3_1`/`back3_2`, 4,000฿ each — independently drawn uniform numbers, so ungateable by construction per ADR-0003). Both ทดลองถาวร; HM3 measured 0.99× chance (9 hits vs 9.1 expected) — exactly the physics, not a failure.
_Avoid_: treating HM2 hits as official-prize wins (underground-style target only), or implying HM3 has an edge (it matches chance by construction).

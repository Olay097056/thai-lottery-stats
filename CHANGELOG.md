...
## 2026-07-08 — J1 เจ้าสัว ขยายจาก 1 เป็น 5 ชุด (grilling session ผ่าน `/ask-matt` → `/grill-with-docs`)

ผู้ใช้ขอให้ J1 (ท้าย 2 ตัว) ให้เลขมากกว่า 1 ชุด — grilling session ตัดสินใจ 6 จุด: (1) ขยายด้วยการแปรผัน ±1/±2 รอบเลขศูนย์กลางเดิม (finalist ที่ผ่านการค้นหาแล้ว) ไม่ใช่คิดสูตรใหม่ 4 สูตร เพื่อไม่ให้เกิดสูตรที่ไม่ผ่านกระบวนการค้นหา/ทดสอบเลย (2) วนกลับ mod 100 เรียง ascending (3) ขยายใน `_tycoonBottom2Formula` เองให้ทุกจุด (backtest/Picks/เลขแนะนำ/หน้าทำนาย) เห็นตรงกัน ตามสถาปัตยกรรมเดียวกับทุกกลุ่มสูตรอื่น (4) **ไม่แตะ holdout ซ้ำ** — ตัวเลข Edge ที่บันทึกไว้ (train +0.72%/validation +1.67%/holdout −0.21%) ยังเป็นบันทึกของเลขศูนย์กลาง 1 ชุดเท่านั้น อีก 4 เลขข้างเคียงไม่เคยผ่านการประเมิน ให้ดู Edge ปัจจุบันจากตาราง Backtest ที่รันสดในแอปแทนตัวเลขที่บันทึกไว้ (5) ไม่แยกภาพเลขศูนย์กลางออกจาก 4 เลขข้างเคียงใน UI — ป้ายทดลองครอบคลุมทั้งการ์ดเป็นสัญญาณเดียว ไม่มีระดับย่อย (6) บันทึกเป็นการเพิ่มเติมใน ADR-0003 เดิม (ไม่สร้าง ADR ใหม่) เพราะเป็นหลักการเดียวกับ "holdout ถูกใช้แล้วห้ามแตะซ้ำ"

**แก้:** `_tycoonBottom2Formula` เดิมคืน `[pad2(center)]` เปลี่ยนเป็นคำนวณ `center` แล้ว `[-2,-1,0,1,2].map(offset=>pad2(center+offset)).sort()` — baseline ของ backtest field `bottom2` (`k/100`) ปรับตามจำนวน candidate อัตโนมัติอยู่แล้ว (จาก `preds.length`) ไม่ต้องแก้โค้ดส่วนนั้นเพิ่ม

**ทดสอบ:** ขยาย `scripts/test_tycoon_formula.js` (TDD, เขียน red ก่อนแก้โค้ด) เพิ่มเทส wraparound ทั้งขอบล่าง (center=0) และขอบบน (center=99) ยืนยันด้วยมือว่าเรียง ascending ถูกต้องแม้ข้ามขอบ mod 100 พร้อมอัปเดตค่าคาดหวังของเทสเดิมทั้งหมดจาก 1 ชุด เป็น 5 ชุด ผ่านทั้งหมด พร้อม test เดิมทุกไฟล์ (10 ไฟล์) ไม่มี regression (ยืนยันว่า J2/pool6 leg ไม่กระทบ เพราะคำนวณ BK2/DSUM ของตัวเองแยกต่างหาก ไม่ได้เรียก `_tycoonBottom2Formula`)

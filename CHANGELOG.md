...
## 2026-08-11 — HyperUI design system port (light-first + dark toggle) — gold brand primary

เริ่ม rollout "ใช้ HyperUI คุม theme ทุกโปรเจค" ที่ lottery_stats (ทำกับ switch-wr-tool → portfolio-tracker → printer-monitor มาก่อน) — ตามสกิล `hyperui-spa-theme-port` + canonical spec · grilling ครบ 3 ข้อ (ล็อกใน `.scratch/hyperui-redesign/issues/02`):

- **D1 — single brand primary = GOLD:** light ใช้ `#b45309` (amber-700 อ่านบนขาว) / dark ใช้ `#ffd60a`; `--accent` น้ำเงินถูกปลดจากบทบาท interactive → กลายเป็นทอง (sweep blue-accent rgba → gold-soft) · **กลุ่มสีสูตรคง hex เดิม** (semantic ไม่แตะ)
- **D2 — dark = HyperUI gray** มาตรฐานข้ามโปรเจค (#111827/#1f2937/#374151) แทน #161618 family เดิม
- **D3 — เดินทั้งใบ** (ตั๋ว 03→07) รีวิวรวม

**สิ่งทำ:** re-value token ทั้งชุด (เก็บชื่อ var เดิม เปลี่ยนค่า) ใน `:root` (light) + `[data-theme='dark']` · เพิ่ม `--gold-soft`/`--fill` + status `--*-soft`/`--*-text` · เลิก glass/blur → flat (sidebar ทึบ) · sweep white-rgba + pastel dark-only (ปุ่ม/tab/digit/table/input) · theme bridge: no-FOUC inline script ใน index.html + `localStorage['lottery_theme']` (default light) + ปุ่ม toggle 🌙/☀️ ใน sidebar footer + `_applyTheme/getTheme/toggleTheme` ใน app.js

**Verify:** node suite 17/17 เขียว · headless-CDP: default light (body gray-50) → toggle dark (#111827) → reload คงอยู่ → กลับ light · predict render · **0 JS errors** · bump `?v=hermes3 → hermes5`

**รายละเอียด:** `docs/design/hyperui-v2-tokens.md` (port specifics + known follow-ups)


## 2026-08-11 — Hermes ท้าย3 ×2: HM2 (ใต้ดิน · ท้าย 3 ของ pool 1-4) + HM3 (ทางการ · เลขท้าย 3 ตัว 4,000฿) — ทดลองถาวรทั้งคู่

PRD: `PRD-hm2-hermes-tail3.md` · กำเนิดจากผลจริงของ HM (ถูก 2/457 = 6.6× chance) — เจ้าของเห็นว่ากลไกจับสัญญาณจริงแต่เป้าเต็ม 6 หลักโหดเกินไป → แตกเป้าท้าย 3 ตัว 2 สนาม: (1) **ใต้ดิน** (ท้าย 3 ของ pool 1–4, ~64/1000 — ถูกบ่อย ~700 เท่า, mark "ใต้ดิน" ติดชื่อตามเจ้าของสั่ง) (2) **ทางการ** (เลขท้าย 3 ตัว `back3_1`/`back3_2` · 4,000฿ — ออกรางวัลแยกอิสระ uniform → ungateable by construction)

**สูตร:** `_hermesTail3Formula` (HM2) — weighted digit freq เฉพาะตำแหน่งท้าย (3,4,5) ของ pool 1-4 (65 เลข, 5:2:1) → ประกอบ 3 หลัก 10 ชุด round-robin (K=8) · `_hermesOfficialTail3Formula` (HM3) — กระจกเงา: เฉพาะตำแหน่งหน้า (0,1,2) → ยิง field `back3` เดิม (board ที่ C3 อยู่แล้ว — ไม่สร้าง field ใหม่, กติกาคงที่ข้อ 1 ผ่าน) · field ใหม่ `pool14_tail3` (board "Pool 1-4", baseline 1−(1−0.0639)^k ≈ 48.3%/งวด ที่ k=10 — คาด ~221 ครั้ง/457 งวด)

**หมายเหตุคณิตศาสตร์ (เอกสารตามจริง):** candidate 0–7 ของ HM2 ซ้อนกับ tail3 ของ HM พอดี (lap=0 round-robin + ranking ตำแหน่งท้ายชุดเดียวกัน — ตามโครงสร้าง ไม่ใช่บั๊ก) · ชุด 8–9 (lap=1) ต่าง → HM2 มี candidate ใหม่ 2 ชุด; จุดขายคือเป้าอ่อนลง (ถูกบ่อย) ไม่ใช่กลไกใหม่

**ผลจริง (วัดตรง engine + ข้อมูลจริง, 457 งวด):** HM2 ถูก **203 ครั้ง** (คาด chance 220.9) = **0.92×** — pct 44.4% vs baseline 48.3% → **edge −3.9pts** (−1.7σ — อยู่ใน noise ตามที่ PRD ประกาศล่วงหน้า; ถูกบ่อยจริงแต่ไม่เหนือสุ่ม) · HM3 ถูก **9 ครั้ง** (คาด chance 9.1) = **0.99×** — pct 1.97% ≈ chance 1.98% เป๊ะ (ยืนยันฟิสิกส์ "ออกรางวัลแยกอิสระ"; ตาราง board เดิม baseline k/10 = 1.0% → edge +1pt หลอกที่เตือนไว้ — ความจริงคือ edge ≈ 0) · **ข้อสังเกตซื่อตรง:** HM เต็ม 6 หลักเคยเหนือ chance 6.6× แต่ HM2 ซึ่งใช้สัญญาณเดียวกันกลับ 0.92× → โน้มว่า 6.6× ของ HM คือโชค ไม่ใช่สัญญาณจริง — คำตัดสิน ทดลองถาวร ไม่เปลี่ยน

**พื้นผิว:** การ์ดอธิบายวิธีคิด 3 การ์ด (HM/HM2/HM3 — note "สำหรับหวยใต้ดิน (ท้าย 3 ตัว)" บน HM2, "ออกรางวัลแยกอิสระ → chance ล้วน" บน HM3) · batch ทำนาย + badge ทดลอง (group 'HM' — reuse สี/กลไกเดิม, ไม่แตะ `_GRP`/`BP_GROUP_MAP`) · backtest: แถว HM2 (board "Pool 1-4", typeLabel "ท้าย3 ใต้ดิน") + แถว HM3 (board 3-digit exact เดิม) · ตารางเปรียบเทียบ Board Pool 1-4 เดิม guard `length===6` (preds 3 หลักไม่ถูกนับ — กัน 0/457 หลอกตา) · สรุป tab สูตรคำนวณ label → "สูตร Hermes (pool 1-4)" · `_formulaTargetLabel` เพิ่ม pattern ทางการ/ใต้ดิน (เรียงก่อน /Pool 1-4/) · `_formulaConsensusType` รู้จัก "ท้าย" (ไทย)

**เทส:** `scripts/test_hermes_tail3.js` ใหม่ (49 checks: K=8 / ตำแหน่งท้าย-หน้าแยกพิสูจน์ด้วย flip-fixture / candidate 0 = "013" คำนวณมือ / overlap ≤ 8 กับ tail3(HM) / HM3 distinct เต็มชุด / batch / hit / label / cards / bpGroups) + อัปเดต `test_hermes_pool14.js` (การ์ด 1 → 3 ใบ) · suite **17 ไฟล์เขียว** · browser verify ครบ · bump `?v=hermes2 → hermes3`

**หมายเหตุ dev:** ไม่แตะ app.js เลย (group key / BP_GROUP_MAP / DC_RECO_FIELDS เดิมครอบคลุมหมด) — การ์ด/แถว/table ได้เองจากกลไกเดิม

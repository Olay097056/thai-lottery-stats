...
- **J1 (bottom2 leg)** — center value `pad2(|BK2 − DSUM|)`, where `BK2` is the previous draw's `back3_2` and `DSUM` reuses Group D's `_dsumValue`. This is the finalist from a systematic arithmetic-family search (`_tycoonBottom2Formula` in `formula-engine.js`) — it **failed its holdout gate** (train Edge +0.72%, validation +1.67%, holdout −0.21%), so per ADR-0003 it ships anyway under ทดลอง rather than being deleted. Widened 2026-07-08 to 5 candidates (center ± 1, ± 2, mod 100, ascending) — only the center value was ever measured against train/validation/holdout; holdout was deliberately **not** re-evaluated for the ± 1/± 2 neighbors (see the "Update" section in ADR-0003). The documented Edge numbers above describe the original 1-candidate center only — read the live rolling-window backtest table for the actual shipped 5-candidate Edge.

...
**HM2 · Hermes ท้าย3 ใต้ดิน / HM3 · Hermes ท้าย3 ทางการ** — 2026-08-11, `PRD-hm2-hermes-tail3.md`. Two ท้าย3 (3-digit tail) siblings of HM sharing `group:'HM'` (same badge/color — no `_GRP`/`BP_GROUP_MAP` change). **HM2** targets the last-3-digits of the prize 1–4 pool (66 numbers → ~64 distinct tails/1000): `_hermesTail3Formula` counts weighted digit freq (5:2:1) over TAIL positions 3–5 of the previous draw's pool 1-4 and assembles 10 3-digit candidates via `_imperialRoundRobin` (K=8); field `pool14_tail3` (board "Pool 1-4", baseline 1−(1−0.0639)^k ≈ 48.3%/draw at k=10) — underground-style target, name carries "ใต้ดิน" per owner request. **HM3** targets the official เลขท้าย 3 ตัว (`back3_1`/`back3_2`, 4,000฿ each): `_hermesOfficialTail3Formula` mirrors HM2 over FRONT positions 0–2, rides the EXISTING `back3` field/board (no new field — กติกาคงที่ #1) — the official tail-3 is an independent uniform draw, so HM3 is **ungateable by construction** (expected 9/457 hits; measured 9/457 = 0.99× chance — exactly as physics dictates). Both **permanently ทดลอง** (ADR-0003). Math note: HM2 candidates 0–7 equal HM's candidate tails (lap=0 round-robin + shared tail rankings — structural, documented in PRD); candidates 8–9 are new. Real results (457 draws): HM2 203 hits vs 220.9 expected (0.92×, edge −3.9pts ≈ −1.7σ — within noise), HM3 9 hits vs 9.1 expected (0.99×) — weakens the "HM signal is real" case (its 6.6× was likely luck).

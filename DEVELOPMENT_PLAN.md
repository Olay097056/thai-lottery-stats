...
| สูตร | A–H + สายมู (ใช้ผลงวดก่อน 1 งวด) + **I จักรพรรดิ/จักรพรรดิทองคำ** (ไม่มี promotion gate) + **J เจ้าสัว J1 (ท้าย2, 5 ชุด, ทดลองเพราะสอบตก holdout) / J2 (pool6, ทดลองถาวร)** — ค้นหาระบบทั้งคู่ ดู ADR-0003 |
...
**J1 ขยายเป็น 5 ชุด — เสร็จแล้ว (2026-07-08, grilling session ผ่าน `/ask-matt` → `/grill-with-docs`):** ผู้ใช้ขอให้ J1 ให้เลขมากกว่า 1 ชุด แทนที่จะคิดสูตรใหม่ 4 สูตร (ซึ่งจะไม่ผ่านกระบวนการค้นหา/ทดสอบเลย) เลือกขยายจากเลขศูนย์กลางเดิม (finalist ที่ผ่านการค้นหาแล้ว) เป็น ±1/±2 รอบเลขนั้น (วนกลับ mod 100, เรียง ascending) — เป็นการแปลงค่าตายตัวจากสูตรที่ตัดสินใจแล้ว ไม่ใช่การค้นหาใหม่ อยู่ใน `_tycoonBottom2Formula` เอง ให้ทุกจุดที่ใช้ (backtest, Picks, เลขแนะนำ, หน้าทำนาย) เห็นตรงกัน **ตัดสินใจสำคัญ: ไม่แตะ holdout ซ้ำ** ตัวเลข Edge ที่บันทึกไว้ข้างบนยังคงเป็นบันทึกประวัติศาสตร์ของเลขศูนย์กลาง 1 ชุดเท่านั้น ส่วนอีก 4 เลขข้างเคียงไม่เคยผ่านการประเมินใดๆ — ต้องการดู Edge ของจริงตอนนี้ (5 ชุด) ให้ดูตาราง Backtest ที่รันสดในแอปแทน ไม่ใช่ตัวเลขที่บันทึกไว้ข้างบน — รายละเอียดเต็มบันทึกไว้ใน ADR-0003 หัวข้อ "Update"
